...
int main(int argc, char **argv)
{
    ...
    asm_output.writeInit(); // writes bootstrap code that initialises SP and starts Sys.init
    ...
    for (vector<string>::size_type i = 0; i != filename_list.size(); ++i)
    {
        ...
        bool if_functions = vm_input.hasFunctions();
        ...
        while ( vm_input.hasMoreCommands() )
        {
            ...
            else if (vm_input.commandType() == "C_LABEL")
            {
                if (if_functions)  // checks if input is file(s) or a directory path
                    asm_output.writeLabel(vm_input.arg1(), function_name);
                else
                    asm_output.writeLabel( vm_input.arg1() );
            }
            else if (vm_input.commandType() == "C_GOTO")
            {
                if (if_functions)
                    asm_output.writeGoto(vm_input.arg1(), function_name);
                else
                    asm_output.writeGoto( vm_input.arg1() );
            }
            else if (vm_input.commandType() == "C_IF")
            {
                if (if_functions)
                    asm_output.writeIf(vm_input.arg1(), function_name);
                else
                    asm_output.writeIf( vm_input.arg1() );
            }

            else if (vm_input.commandType() == "C_CALL")
                asm_output.writeCall( vm_input.arg1(), vm_input.arg2() );
            else if (vm_input.commandType() == "C_FUNCTION")
            {
                function_name = vm_input.arg1();
                cout << function_name << endl;
                asm_output.writeFunction( vm_input.arg1(), vm_input.arg2() );
            }
            else if (vm_input.commandType() == "C_RETURN")
                asm_output.writeReturn();
        }
    }
